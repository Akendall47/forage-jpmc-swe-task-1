...
class App(object):
    ...
    def handle_query(self, x):
        """ Takes a query with a stock symbol and returns a list of the
            10 best bids and asks.
        """
        symbol = x['stock']
        book = self._current_book_1 if symbol == 'ABC' else self._current_book_2
        t, bids, asks = next(book)
        print('Query received @ t%s' % t)
        return {
            'bids': [{'price': price, 'size': size} for price, size, age in bids[:10]],
            'asks': [{'price': price, 'size': size} for price, size, age in asks[:10]],
        }
